...
def map_row(applicant: Dict[str, Any]) -> Dict[str, Any]:
    ...
    university = applicant.get("University") or applicant.get("university")
...
# listing all rows to be inserted/updated and then performing queries to demonstrate the loaded data.
    with conn.cursor() as cur:
        ...
        print(f"Number of applicants for Fall 2026: {count_result[0]}")
        ...
        cur.execute("""
                ...
                """, ('Fall 2026', 'Accepted'))
        Fall2026_accepted = cur.fetchone()[0]
        if Fall2026_accepted > 0:
            acceptance_percentage = (Fall2026_accepted / total_count) * 100
        ...
        cur.execute("""
                    SELECT COUNT(*) FROM applicants
                    WHERE university ILIKE %s AND degree ILIKE %s AND program ILIKE %s;
        ...
        cur.execute("""
                    SELECT COUNT(*) FROM applicants
                    WHERE term ILIKE %s AND status ILIKE %s AND university ILIKE ANY (ARRAY[%s, %s, %s, %s]) AND degree ILIKE %s AND program ILIKE %s;
